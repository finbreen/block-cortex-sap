...
view: sales_orders {
  ...
  dimension: distribution_channel_name {
    ...
    hidden: no
    sql: CASE
          WHEN ${distribution_channel_vtweg} = "10" THEN "Geräte"
          WHEN ${distribution_channel_vtweg} = "20" THEN "Systeme"
          WHEN ${distribution_channel_vtweg} = "30" THEN "OEM"
          WHEN ${distribution_channel_vtweg} = "40" THEN "Ersatzteile"
          WHEN ${distribution_channel_vtweg} = "50" THEN "Service"
          WHEN ${distribution_channel_vtweg} = "60" THEN "Ersatzteile RSF"
          ELSE "Undefined"
          END ;;  }
  ...
  dimension: canceled_order {
    type: string
    sql: IF(COALESCE(${rejection_reason_abgru}) != '','Canceled','NotCanceled') ;;
    hidden: no
  }
